...
------
  QMainWindow
    ...
    │     Tab 3: AI Assistant   → AIWindow
    │     Tab 4: ESG Scenarios  → EsgWindow
    │
    └── Status bar
          API connectivity indicator (checked every 10 s)

All tabs that call the API share a single ALMApiClient instance constructed
here and passed into each child widget.  EsgWindow runs locally and does not
...
from frontend.desktop.windows.esg_window import EsgWindow
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, client: ALMApiClient, parent=None) -> None:
        ...
        self._tabs.addTab(self._esg_tab,     "ESG Scenarios")
        ...
        status_bar.addPermanentWidget(self._api_status_label)
