...
class AIChatRequest(BaseModel):
    ...
    # --- Optional external LLM config (used only for RegulatoryResearchAgent) ---
    # When absent, the regulatory_research route is unavailable.

    external_provider: Optional[str] = Field(
        None,
        description=(
            "AI provider for the external LLM (used by RegulatoryResearchAgent only). "
            "Omit to disable regulatory research routing."
        ),
    )
    external_model: Optional[str] = Field(
        None,
        description="Model ID for the external LLM.",
    )
    external_base_url: Optional[str] = Field(
        None,
        description="Base URL for the external LLM if using openai_compatible provider.",
    )
...
class AIChatResponse(BaseModel):
    ...
    agent_trace: list[str] = Field(
        default_factory=list,
        description="Routing trace for this turn, e.g. ['router', 'bpa'].",
    )
