...
class Fund:
    ...
    investment_strategy : BaseStrategy
        Computes rebalancing orders and forced sells. Injected per CLAUDE.md Rule 5.
        Accepts InvestmentStrategy (SAA rebalancing) or BuyAndHoldStrategy (BPA MA).
    initial_cash : float
    ...
    def __init__(
        ...
        self._asset_model:         AssetModel   = asset_model
        self._liability_model:     BaseLiability = liability_model
        self._investment_strategy: Any           = investment_strategy
    ...
    def step_time_with_liability(
        ...
        """
        Advance the fund by one period using pre-computed liability results.

        Identical to ``step_time`` but skips the liability model call (steps 1).
        Used by ``StochasticRun._execute_vectorised``, which pre-computes all
        N scenarios' liability cashflows in one ``batch_step`` call and then
        feeds each scenario's result here for the asset-side computation.

        ``step_time`` itself is unchanged — deterministic and liability-only
        runs continue to use it unchanged.

        Parameters
        ----------
        scenario : AssetScenarioPoint
        cashflows : LiabilityCashflows
            Pre-computed by ``liability_model.batch_step`` for this scenario.
        decrements : Decrements
            Pre-computed by ``liability_model.batch_step`` for this scenario.
        ...
        # Steps 2–7 are identical to step_time; liability step is skipped.
        asset_cfs    = self._asset_model.collect_cashflows(scenario)
        asset_income = (
            asset_cfs.coupon_income
            + asset_cfs.dividend_income
        ...
        if self._cash_balance < 0.0:
            ...
            for order in forced:
                asset = self._asset_model.get_asset(order.asset_id)
                trade = asset.rebalance(order.target_value, scenario)
                self._cash_balance -= trade
            if self._cash_balance < -1.0:
                self._logger.warning(
                    "Timestep %d: cash shortfall of £%.2f remains after forced sells "
                    "(AC constraint may be preventing required sales).",
                    t, -self._cash_balance,
