...
class RunType(str, Enum):
    ...
    BPA:
        BPA-specific ALM run using a hybrid monthly/annual timestep and a
        buy-and-hold MA portfolio strategy. Computes pre-MA and post-MA BEL,
        loads and saves Ifrs17State per cohort, and writes an MA governance
        summary alongside the main results (DECISIONS.md §21, §27, §46).
    """
...
class BPAInputConfig(BaseModel):
    """
    BPA-specific input file paths. Required when run_type='bpa'.

    bpa_model_points_path:
        Path to the combined BPA model points CSV.  All four population types
        (in_payment, deferred, dependant, enhanced) are stored in a single file
        discriminated by the 'population_type' column.  The worker splits the
        file into four DataFrames and routes each to the matching liability class.

    bpa_deals_path:
        Path to bpa_deals.csv — the BPA deal registry.
        Required columns: deal_id, deal_type, inception_date, deal_name, ma_eligible.
        Loaded once per run; deal_ids are cross-referenced against model points.

    mortality_dir:
        Directory containing the four mortality CSV files:
        S3PMA.csv, S3PFA.csv, CMI_2023_M.csv, CMI_2023_F.csv.
        Each file must cover ages 16–120 (105 rows).

    fundamental_spread_path:
        Path to pra_fundamental_spread.csv — the PRA Fundamental Spread table.
        Required columns: rating, seniority, tenor_lower, tenor_upper, fs_bps.
        Must also carry governance comment headers:
          # effective_date: YYYY-MM-DD
          # source_ref: <PRA publication>

    asset_cashflows_path:
        Path to bpa_asset_cashflows.csv — annual cashflows net of default.
        Required columns: t (int, year), asset_id (str), cf (float).
        One row per (asset, year); used by MACalculator for the cashflow
        matching test (Condition 4) and for FS-weighted MA benefit computation.
    """
    bpa_model_points_path:    Path = Field(
        ...,
        description="Combined BPA model points CSV (all 4 population types)."
    )
    bpa_deals_path:           Path = Field(
        ...,
        description="BPA deal registry CSV (bpa_deals.csv)."
    )
    mortality_dir:            Path = Field(
        ...,
        description="Directory containing S3PMA, S3PFA, CMI_2023_M, CMI_2023_F CSVs."
    )
    fundamental_spread_path:  Path = Field(
        ...,
        description="PRA Fundamental Spread table CSV (pra_fundamental_spread.csv)."
    )
    asset_cashflows_path:     Path = Field(
        ...,
        description="Annual bond cashflows net of default CSV (t, asset_id, cf)."
    )

    @field_validator(
        "bpa_model_points_path", "bpa_deals_path",
        "fundamental_spread_path", "asset_cashflows_path",
        mode="before",
    )
    @classmethod
    def resolve_and_check_file(cls, v) -> Path:
        p = Path(v).resolve()
        if not p.exists():
            raise ValueError(f"BPA input file not found: {p}")
        return p

    @field_validator("mortality_dir", mode="before")
    @classmethod
    def resolve_and_check_mortality_dir(cls, v) -> Path:
        p = Path(v).resolve()
        if not p.exists():
            raise ValueError(f"BPA mortality directory not found: {p}")
        if not p.is_dir():
            raise ValueError(f"mortality_dir must be a directory: {p}")
        return p
...
class InputSourcesConfig(BaseModel):
    ...
    model_points:
        ...
        Not required for BPA runs — BPA loads model points from bpa_inputs.bpa_model_points_path.
    ...
    bpa_inputs:
        BPA-specific file paths (model points, deals, mortality, FS table,
        asset cashflows). Required when run_type='bpa'.
        See BPAInputConfig for full field descriptions.
    """
    model_points:       Optional[ModelPointSourceConfig] = Field(
        default=None,
        description=(
            "Model point data source: database or file. "
            "Required for LIABILITY_ONLY, DETERMINISTIC, and STOCHASTIC runs. "
            "Not used for BPA runs — BPA loads its own model points via bpa_inputs."
        ),
    )
    ...
    bpa_inputs:         Optional[BPAInputConfig] = Field(
        default=None,
        description="BPA-specific input paths. Required when run_type='bpa'.",
    )
...
class StochasticConfig(BaseModel):
    ...
    use_vectorised:     bool           = Field(
        default=False,
        description=(
            "Use the JAX-vectorised execution path (DECISIONS.md §32). "
            "When False (default) the existing serial loop runs unchanged. "
            "When True, liability.batch_step() processes all scenarios in "
            "one compiled JAX call per timestep. Only Conventional products "
            "have a JAX override; other liability classes fall back to the "
            "default loop. Requires JAX to be installed."
        ),
    )


class BPAConfig(BaseModel):
    """
    Parameters specific to BPA runs. Ignored for other run types.

    monthly_years:
        Number of years at the start of the projection that use monthly
        time steps. After this point the calendar switches to annual steps.
        The ProjectionCalendar produces monthly_years × 12 monthly periods
        followed by (projection_term_years − monthly_years) annual periods.
        Must not exceed projection_term_years.
        Default: 10 (first decade monthly, remainder annual).
    """
    monthly_years: int = Field(
        default=10,
        ge=0,
        description=(
            "Years of monthly steps at projection start before switching to "
            "annual steps. Must be <= projection_term_years."
        ),
    )
...
class RunConfig(BaseModel):
    ...
    bpa:           Optional[BPAConfig]        = Field(
        default=None,
        description="BPA-specific parameters (hybrid calendar). Used when run_type=bpa."
    )
